...
<body class="boxed-size">
                    ...
                    <div id="map-wrapper">
                        ...
                        <div id="layers-sidebar">
                            ...
                            <div id="layers-sidebar-content" class="p-3">
                                <div class="mb-3"><h6>Official Incidents</h6><div class="form-check form-switch"><input class="form-check-input layer-toggle" type="checkbox" role="switch" id="official-perimeters" checked><label class="form-check-label legend-item" for="official-perimeters"><span class="legend-icon"><i class="fas fa-circle text-danger"></i></span>< 24h (by Size)</label></div><div class="legend-item ms-2"><span class="legend-icon"><i class="fas fa-circle text-warning"></i></span>< 3d (by Size)</div><div class="legend-item ms-2"><span class="legend-icon"><i class="fas fa-circle text-info"></i></span>> 3d (by Size)</div></div>
    ...
    <script>
        ...
        function setupEventListeners() {
            ...
            document.getElementById('calculate-route-btn').addEventListener('click', calculateAndSaveRoute); document.getElementById('clear-markers-btn').addEventListener('click', clearRouteMarkers); document.getElementById('saved-routes-list').addEventListener('click', handleSavedRouteClick);
        }
        ...
        async function loadOfficialPerimeters() {
            ...
            try {
                const response = await axios.get('/api/wildfire-perimeters'); if (!response.data || !response.data.features) return; const now = Date.now(), oneDay = 86400000, threeDays = 3 * oneDay;
                L.geoJSON(response.data, { onEachFeature: (feature, layer) => {
                    const props = feature.properties; const age = now - (props.poly_PolygonDateTime || props.attr_ModifiedOnDateTime_dt);
                    let color = '#0dcaf0'; if (age < oneDay) color = '#dc3545'; else if (age < threeDays) color = '#fd7e14';
                    let radius = Math.max(5, Math.min(20, 5 + Math.log((props.poly_GISAcres || 0) + 1))); const bounds = layer.getBounds();
                    if (bounds.isValid()) {
                ...
                initializeFireSearch(); if (is3D) synchronizeLayersToCesium();
            } catch (error) { console.error("Failed to load official perimeters:", error); } finally { document.getElementById('main-loader').classList.add('d-none'); }
        }
        ...
        function initializeFireSearch() { if (map.fireSearchControl) map.removeControl(map.fireSearchControl); map.fireSearchControl = new L.Control.Geocoder({ defaultMarkGeocode: false, geocoder: L.Control.Geocoder.nominatim(), query: 'Search location or fire...', placeholder: 'Search...' }).on('markgeocode', e => map.setView(e.geocode.center, 10)).addTo(map); }
