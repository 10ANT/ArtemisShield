...
<head>
    ...
    <style>
        ...
        .transcript {
            ...
            white-space: pre-wrap; /* Ensures line breaks in transcript are respected */
    ...
    <div class="container">
        ...
        <div class="section">
            ...
            <div class="transcript">
                <p>"{{ $report->transcript ?? 'Transcript not available.' }}"</p>
            </div>
        </div>
        ...
        {{-- START: UPDATED SUGGESTIONS BLOCK --}}
        @if(!empty($report->ai_suggested_actions) && is_array($report->ai_suggested_actions))
            <div class="section">
                <h2>AI-Powered Suggested Actions</h2>
                <div class="suggestions">
                    <ul>
                        @php
                            // This logic robustly handles both possible data structures:
                            // 1. A direct array of suggestions: [ ['suggestion' => '...'], ... ]
                            // 2. A nested structure: { "suggestions": [ ... ] }
                            $suggestions = isset($report->ai_suggested_actions['suggestions']) && is_array($report->ai_suggested_actions['suggestions'])
                                           ? $report->ai_suggested_actions['suggestions']
                                           : $report->ai_suggested_actions;
                        @endphp

                        @forelse($suggestions as $action)
                            {{-- Ensure the item is a valid array with a suggestion key before displaying --}}
                            @if(is_array($action) && !empty($action['suggestion']))
                                <li>{{ $action['suggestion'] }}</li>
                            @endif
                        @empty
                            <li>No actionable suggestions were generated by the AI for this report.</li>
                        @endforelse
                    </ul>
                </div>
            </div>
        @endif
        {{-- END: UPDATED SUGGESTIONS BLOCK --}}
